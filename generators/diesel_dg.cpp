...
diesel_dg::diesel_dg(MODULE *module)
{
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_int16, "give_only_reactive", PADDR(give_only_reactive), PT_DESCRIPTION, "if the reactive is not enough the generator give only reactive in price of active ",
			PT_double, "given_reactive[VAr]", PADDR(given_reactive),PT_DESCRIPTION,"given reactive power",
...
TIMESTAMP diesel_dg::sync(TIMESTAMP t0, TIMESTAMP t1) 
{
	...
	if (Gen_type != DYNAMIC)
	{
		...
		//price=0.678;//(0.01/1000)*TotalPowerOutput.Re()*TotalPowerOutput.Re()*((double)dt/3600) +(11.0/1000)*TotalPowerOutput.Re()*((double)dt/3600)+0.145;
