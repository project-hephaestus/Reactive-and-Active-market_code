...
//files to take information about reactive power for each town
  ofstream myfile1;
  ofstream myfile;
  ofstream myfile2;
  ofstream myfile3;
...
/**/		PT_object, "network", PADDR(network), PT_DESCRIPTION, "the comm network used by object to talk to the market (if any)",
			...
			PT_int64, "type", PADDR(type),PT_DESCRIPTION, "2 for auction town 1 for second level",
...
int auction::init(OBJECT *parent)
{
	...
	unsigned int i = 0;
	myfile.open ("Mar.txt");
	  myfile1.open ("Market_1town1.txt");
	  myfile2.open ("Market_1town2.txt");
	  myfile3.open ("Market_1town3.txt");
	  myfile4.open ("Market_1town4.txt");
	  myfile5.open ("Market_1town5.txt");
	  myfile6.open ("Market_1town6.txt");
	  myfile7.open ("Market_1town7.txt");
	  myfile8.open ("Market_1town8.txt");
	  myfile9.open ("Market_1town9.txt");
	  myfile10.open ("Market_1town10.txt");
	  myfilec1.open ("Market_1townc1.txt");
      myfilec2.open ("Market_1townc2.txt");
...
TIMESTAMP auction::presync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	if (t1>=clearat)
	{
		...
		if(!strcmp(hdr->name,"Market_1town1"))
		{
		myfile1<<timebuf << " ";
		}
		else if(!strcmp(hdr->name,"Market_1town2"))
		{
		myfile2<<timebuf << " ";
		}
		else if(!strcmp(hdr->name,"Market_1town3"))
		{
		myfile3<<timebuf << " ";
		}
		else if(!strcmp(hdr->name,"Market_1town4"))
		{
		myfile4<<timebuf << " ";
		}
		else if(!strcmp(hdr->name,"Market_1town5"))
		{
		myfile5<<timebuf << " ";
		}
		else if(!strcmp(hdr->name,"Market_1town6"))
		{
		myfile6<<timebuf << " ";
		}
		else if(!strcmp(hdr->name,"Market_1town7"))
		{
		myfile7<<timebuf << " ";
		}
		else if(!strcmp(hdr->name,"Market_1town8"))
		{
		myfile8<<timebuf << " ";
		}
		else if(!strcmp(hdr->name,"Market_1town9"))
		{
		myfile9<<timebuf << " ";
		}
		else if(!strcmp(hdr->name,"Market_1town10"))
		{
		myfile10<<timebuf << " ";
		}
		else if(!strcmp(hdr->name,"Market_1city1"))
		{
		myfilec1<<timebuf << " ";
		}
		else if(!strcmp(hdr->name,"Market_1city2"))
		{
		myfilec2<<timebuf << " ";
		}
...
/*	if(counter>=30)
	{
     ...
     climates = gl_find_objects(FL_NEW,FT_CLASS,SAME,"generator_controller",FT_END);
	 OBJECT *firstqq = gl_find_next(climates,NULL);
	 OBJECT *iqq;
		for(iqq=firstqq;iqq!=NULL;iqq=iqq->next)
		{
			if(gl_object_isa(iqq,"generator_controller"))
			{


				 int16 *bid_now=gl_get_int16_by_name(iqq,"bid_from_now");
					if(*bid_now==0 )
					{
						printf("%s",iqq->name);
					gl_set_value_by_name(iqq,"bid_from_now","1");
	...
	if(counter2>=35)
	{
	 static FINDLIST *climates = NULL;
     climates = gl_find_objects(FL_NEW,FT_CLASS,SAME,"generator_controller",FT_END);
	 OBJECT *first = gl_find_next(climates,NULL);
	 OBJECT *i;
		for(i=first;i!=NULL;i=i->next)
		{
			if(gl_object_isa(i,"generator_controller"))
			{


				 int16 *bid_now=gl_get_int16_by_name(i,"bid_from_now");
					if(*bid_now==3  )
					{
						printf("%s",i->name);
					gl_set_value_by_name(i,"bid_from_now","1");
...
//// MAKE THE WIND_TURBINE TO FOLLOW THE CURVE OF DEMAND ////////////
/*
	static FINDLIST *w=NULL;
    w=gl_find_objects(FL_NEW,FT_CLASS,SAME,"windturb_dg",FT_END);
    OBJECT *first33= gl_find_next(w,NULL);
    OBJECT *iij;

    for(iij=first33;iij!=NULL;iij=iij->next)
    {
		if(gl_object_isa(iij,"windturb_dg"))
		{
			char x[60];
			double *active=gl_get_double_by_name(iij,"TotalRealPow");

			if((*active/1000) > cleared_frame.buyer_total_quantity )
			{
					double new_wind_bid_quantity= cleared_frame.buyer_total_quantity +0.05*cleared_frame.buyer_total_quantity;
				   //printf("NEW_QUANTITY:%f  %f %f ",cleared_frame.buyer_total_quantity,*active/1000,new_wind_bid_quantity);
					sprintf(x,"%f",new_wind_bid_quantity*1000);
					gl_set_value_by_name(iij,"TotalRealPow",x);
			}
		}
	}
	*/
/*
    static FINDLIST *w2=NULL;
    w2=gl_find_objects(FL_NEW,FT_CLASS,SAME,"diesel_dg",FT_END);
    OBJECT *firs3= gl_find_next(w2,NULL);

	for(iij=firs3;iij!=NULL;iij=iij->next)
    {
		if(gl_object_isa(iij,"diesel_dg"))
		{
			...
			if((*active/1000) > cleared_frame.buyer_total_quantity )
			{
					double new_wind_bid_quantity= cleared_frame.buyer_total_quantity +0.1*cleared_frame.buyer_total_quantity;
					sprintf(x,"%f",new_wind_bid_quantity);
					gl_set_value_by_name(iij,"TotalRealPow",x);
			}
			else
			{
...
///////////////////////// SECOND LEVEL AUCTION CONFIGURATION /////////////////////////////
/*
	char s1[1200];
	char s2[1200];
	char s3[1200];
	...
	char s9[1200];
		...
		static FINDLIST *xy=gl_find_objects(FL_NEW,FT_CLASS,SAME,"controller",FT_END);
		OBJECT *obj1=NULL;
		OBJECT *obj;

		do{
			obj=gl_find_next(xy,obj1);

			if (gl_object_isa(obj, "controller")){


			gl_set_value_by_name(obj->parent,"power_state","1");
			int *state=gl_get_int32_by_name(obj->parent,"power_state");
			//int *priority=gl_get_int32_by_name(obj->parent,"priority");
			//if(*priority==3 && *state)
				//printf("test %d ",*state);
				}
			obj1=obj;
  ...
  if(type==2)
  {
		...
		sprintf(s2,"%f",1.1*(*seller_price));
		sprintf(s6,"%f",asks.getcount());
	//	printf("price:%s",s6);
		sprintf(s7,"%f",current_frame.buyer_total_quantity);
	//	sprintf(s1,"%f",offers.getcount());
		sprintf(s8,"%f",current_frame.seller_total_quantity);


		//condition A
		if(cleared_frame.seller_total_quantity >=cleared_frame.buyer_total_quantity )
		{

		sprintf(s3,"%f",cleared_frame.seller_total_quantity-cleared_frame.clearing_quantity);
		//	printf("supply:%s for the town factory at price:%s  \n",s3,s5);
			gl_set_value_by_name(sec_bidder,"price","0.55");
		...
		else if(cleared_frame.clearing_quantity < cleared_frame.buyer_total_quantity)
		{
				...
				{
					printf("kanw sto second\n\n\n\n\n");
					gl_set_value_by_name(sec_bidder,"quantity",s4);
					gl_set_value_by_name(sec_bidder,"price","0.58");
				...
				//gl_set_value_by_name(new_buyer,"price",s6);
				//gl_set_value_by_name(new_buyer,"quantity",s7);
				//gl_set_value_by_name(bidder,"change","0");
		//printf("demand:%s \n",s4);
...
/////////////////////////// END SECOND LEVEL AUCTION CONFIGURATION //////////////////////


//////////////// START NEW CONFIFURATION /////////////////////
  OBJECT *hdr=OBJECTHDR(this);
  static FINDLIST *x=NULL;
  x=gl_find_objects(FL_NEW,FT_CLASS,SAME,"triplex_meter",FT_END);
  OBJECT *first= gl_find_next(x,NULL);
  OBJECT *i;

  double *reactive_of_each_diesel;
  double total_reactive_of_diesel=0;
  static FINDLIST *y=NULL;
  y=gl_find_objects(FL_NEW,FT_CLASS,SAME,"diesel_dg",FT_END);
  OBJECT *first2= gl_find_next(y,NULL);
  OBJECT *ii;

  double *reactive_of_each_central_meter;
  double total_reactive_of_devices=0;


  double *reactive_of_each_wind;
  double *real_of_each_wind;
  double total_reactive_of_wind=0;
  double total_real_of_wind=0;
  static FINDLIST *z=NULL;
  z=gl_find_objects(FL_NEW,FT_CLASS,SAME,"windturb_dg",FT_END);
  OBJECT *first3= gl_find_next(z,NULL);
  OBJECT *iii;

  for(i=first;i!=NULL;i=i->next)
  {
   if(gl_object_isa(i,"triplex_meter"))
   {

    int16 *central=gl_get_int16_by_name(i,"central");

    if((*central==1))
	{

    reactive_of_each_central_meter= gl_get_double_by_name(i,"measured_reactive_power");  //se Watt
	total_reactive_of_devices=total_reactive_of_devices + *(reactive_of_each_central_meter);
    }
   }
  }

  //printf("%s  devices reactive: %f ",hdr->name, total_reactive_of_devices);
 // myfile<<total_reactive_of_devices<<" ";
  if(!strcmp(hdr->name,"Market_1town1"))
		{

		myfile1<<total_reactive_of_devices<<" ";
		}
		else if(!strcmp(hdr->name,"Market_1town2"))
		{
		myfile2<<total_reactive_of_devices<<" ";
		}
		else if(!strcmp(hdr->name,"Market_1town3"))
		{
		myfile3<<total_reactive_of_devices<<" ";
		}
		else if(!strcmp(hdr->name,"Market_1town4"))
		{
		myfile4<<total_reactive_of_devices<<" ";
		}
		else if(!strcmp(hdr->name,"Market_1town5"))
		{
		myfile5<<total_reactive_of_devices<<" ";
		}
		else if(!strcmp(hdr->name,"Market_1town6"))
		{
		myfile6<<total_reactive_of_devices<<" ";
		}
		else if(!strcmp(hdr->name,"Market_1town7"))
		{
		myfile7<<total_reactive_of_devices<<" ";
		}
		else if(!strcmp(hdr->name,"Market_1town8"))
		{
		myfile8<<total_reactive_of_devices<<" ";
		}
		else if(!strcmp(hdr->name,"Market_1town9"))
		{
		myfile9<<total_reactive_of_devices<<" ";
		}
		else if(!strcmp(hdr->name,"Market_1town10"))
		{
		myfile10<<total_reactive_of_devices<<" ";
		}
		else if(!strcmp(hdr->name,"Market_1city1"))
		{
		myfilec1<<total_reactive_of_devices<<" ";
		}
		else if(!strcmp(hdr->name,"Market_1city2"))
		{
		myfilec2<<total_reactive_of_devices<<" ";
		}
 // myfile <<total_reactive_of_devices<<" ";
 // printf("total_devices se VAr:%f  ",total_reactive_of_devices);


  for(ii=first2;ii!=NULL;ii=ii->next)
  {
   if(gl_object_isa(ii,"diesel_dg"))
   {
   double *price=gl_get_double_by_name(ii,"price");

	   if(*price <= current_frame.clearing_price)
	   {

    reactive_of_each_diesel= gl_get_double_by_name(ii,"TotalReacPow");
	total_reactive_of_diesel=total_reactive_of_diesel + *(reactive_of_each_diesel);
	   }
   }
  }
  //printf("%f ",total_reactive_of_diesel);
  //printf("total_diesel se VAr:%f  ",total_reactive_of_diesel);



  for(iii=first3;iii!=NULL;iii=iii->next)
  {
   if(gl_object_isa(iii,"windturb_dg"))
   {
    //printf("take to reactive from wind:");
	   double *price=gl_get_double_by_name(iii,"price");

	   if(*price <= current_frame.clearing_price)
	   {

	     reactive_of_each_wind= gl_get_double_by_name(iii,"TotalReacPow");
	     total_reactive_of_wind=total_reactive_of_wind + *(reactive_of_each_wind);
	     real_of_each_wind= gl_get_double_by_name(iii,"TotalRealPow");
         total_real_of_wind=total_real_of_wind + *(real_of_each_wind);
	   }
  ...
  myfile <<"wind_reactive:"<<total_reactive_of_wind<<"wind_real:"<<total_real_of_wind<<"diesel:"<<total_reactive_of_diesel<<"total demand"<<current_frame.buyer_total_quantity<<"total reactive demand:"<<total_reactive_of_devices <<"\n";
  //printf("%f ",total_reactive_of_wind);
  //printf("total_wind se VAr:%f  ",total_reactive_of_wind);


  printf("total gen:%f total devices:%f\n\n",total_reactive_of_wind+total_reactive_of_diesel,total_reactive_of_devices);

  if((total_reactive_of_wind+total_reactive_of_diesel) > total_reactive_of_devices )
  {
     //EVERYTHING IS OK
  ...
  {
    //CHECK THE GENERATORS THAT ARE NOT PARTICIPATING IN THE MARKET

		double wanted_reactive = total_reactive_of_devices-(total_reactive_of_diesel+total_reactive_of_wind)/1000;
		static FINDLIST *y=NULL;
		y=gl_find_objects(FL_NEW,FT_CLASS,SAME,"diesel_dg",FT_END);
		OBJECT *first2= gl_find_next(y,NULL);
		OBJECT *ii;

		myfile << "\n"<<"Wanted_reactive"<<wanted_reactive<<"\n";

     //SEARCH FOR DIESEL
		  for(ii=first2;ii!=NULL;ii=ii->next)
		  {

			  if( wanted_reactive >=0 )
			  {
					if(gl_object_isa(ii,"diesel_dg"))
					{

						 double *price=gl_get_double_by_name(ii,"price");
						//printf("diesel:price:%f %f\n",*price, current_frame.clearing_price);
						 if(*price > current_frame.clearing_price)
						 {

							myfile<<"TAKE FROM DIESEL:QUANTITY"<<"\n";
							gl_set_value_by_name(ii,"give_only_reactive","1");
							//printf("name: %s  ",ii->name);

							double *reactive=gl_get_double_by_name(ii,"TotalReacPow");
							char t12[64]; sprintf(t12,"%f",*reactive);
							gl_set_value_by_name(ii,"given_reactive",t12);


							  //myfile <<*price<<ii->name <<" "<<*reactive <<"\n";
									if(!strcmp(hdr->name,"Market_1town1"))
									{
									myfile1<<*price<<ii->name <<" "<<*reactive <<"\n";
									}
									else if(!strcmp(hdr->name,"Market_1town2"))
									{
									myfile2<<*price<<ii->name <<" "<<*reactive <<"\n";
									}
									else if(!strcmp(hdr->name,"Market_1town3"))
									{
									myfile3<<*price<<ii->name <<" "<<*reactive <<"\n";
									}
									else if(!strcmp(hdr->name,"Market_1town4"))
									{
									myfile4<<*price<<ii->name <<" "<<*reactive <<"\n";
									}
									else if(!strcmp(hdr->name,"Market_1town5"))
									{
									myfile5<<*price<<ii->name <<" "<<*reactive <<"\n";
									}
									else if(!strcmp(hdr->name,"Market_1town6"))
									{
									myfile6<<*price<<ii->name <<" "<<*reactive <<"\n";
									}
									else if(!strcmp(hdr->name,"Market_1town7"))
									{
									myfile7<<*price<<ii->name <<" "<<*reactive <<"\n";
									}
									else if(!strcmp(hdr->name,"Market_1town8"))
									{
									myfile8<<*price<<ii->name <<" "<<*reactive <<"\n";
									}
									else if(!strcmp(hdr->name,"Market_1town9"))
									{
									myfile9<<*price<<ii->name <<" "<<*reactive <<"\n";
									}
									else if(!strcmp(hdr->name,"Market_1town10"))
									{
									myfile10<<*price<<ii->name <<" "<<*reactive <<"\n";
									}
									else if(!strcmp(hdr->name,"Market_1city1"))
									{
									myfilec1<<*price<<ii->name <<" "<<*reactive <<"\n";
									}
									else if(!strcmp(hdr->name,"Market_1city2"))
									{
									myfilec2<<*price<<ii->name <<" "<<*reactive <<"\n";
									}

						   //printf("given_reactive: %f  %f\n\n",wanted_reactive,(*reactive));
							wanted_reactive=wanted_reactive - *(reactive);
			 ...
			 else
			 {
				  ...
				  if(gl_object_isa(ii,"diesel_dg"))
			      {
							...
							gl_set_value_by_name(ii,"give_only_reactive","0");
							gl_set_value_by_name(ii,"given_reactive","0");
				   }
   ...
   //SEARCH FOR WIND_TURBINE
	   	static FINDLIST *w=NULL;
	    w=gl_find_objects(FL_NEW,FT_CLASS,SAME,"windturb_dg",FT_END);
	    OBJECT *first3= gl_find_next(w,NULL);
	    OBJECT *iii;
    	...
    	for(iii=first3;iii!=NULL;iii=iii->next)
  		{
					...
					if(gl_object_isa(iii,"windturb_dg"))
		 			{
			 			...
			 			if(*price > current_frame.clearing_price)
						{
							...
							//ON WIND TURBINE
							gl_set_value_by_name(iii,"give_only_reactive","1");

							double *reactive=gl_get_double_by_name(iii,"TotalReacPow");

								///myfile<<*price<<iii->name <<" "<<*reactive <<"\n";
								if(!strcmp(hdr->name,"Market_1town1"))
								{
								myfile1<<*price<<iii->name <<" "<<*reactive <<"\n";
								}
								else if(!strcmp(hdr->name,"Market_1town2"))
								{
								myfile2<<*price<<iii->name <<" "<<*reactive <<"\n";
								}
								else if(!strcmp(hdr->name,"Market_1town3"))
								{
								myfile3<<*price<<iii->name <<" "<<*reactive <<"\n";
								}
								else if(!strcmp(hdr->name,"Market_1town4"))
								{
								myfile4<<*price<<iii->name <<" "<<*reactive <<"\n";
								}
								else if(!strcmp(hdr->name,"Market_1town5"))
								{
								myfile5<<*price<<iii->name <<" "<<*reactive <<"\n";
								}
								else if(!strcmp(hdr->name,"Market_1town6"))
								{
								myfile6<<*price<<iii->name <<" "<<*reactive <<"\n";
								}
								else if(!strcmp(hdr->name,"Market_1town7"))
								{
								myfile7<<*price<<iii->name <<" "<<*reactive <<"\n";
								}
								else if(!strcmp(hdr->name,"Market_1town8"))
								{
								myfile8<<*price<<iii->name <<" "<<*reactive <<"\n";
								}
								else if(!strcmp(hdr->name,"Market_1town9"))
								{
								myfile9<<*price<<iii->name <<" "<<*reactive <<"\n";
								}
								else if(!strcmp(hdr->name,"Market_1town10"))
								{
								myfile10<<*price<<iii->name <<" "<<*reactive <<"\n";
								}
								else if(!strcmp(hdr->name,"Market_1city1"))
								{
								myfilec1<<*price<<iii->name <<" "<<*reactive <<"\n";
								}
								else if(!strcmp(hdr->name,"Market_1city2"))
								{
								myfilec2<<*price<<iii->name <<" "<<*reactive <<"\n";
								}


								char t12[64];
								sprintf(t12,"%f",*reactive);
								gl_set_value_by_name(iii,"given_reactive",t12);
	 		...
	 		else
	 		{
				...
				if(gl_object_isa(iii,"windturb_dg"))
				{
						gl_set_value_by_name(iii,"give_only_reactive","0");
						gl_set_value_by_name(iii,"given_reactive","0");
				}
  ...
  ////////CLOSE THE GENERATORS THAT ARE NOT BIDDING ON THE MARKET AND OPEN THEM WHEN MORE SUPPLY IS NEEDED////////////////////////

	/*static FINDLIST *q=NULL;
    q=gl_find_objects(FL_NEW,FT_CLASS,SAME,"diesel_dg",FT_END);
    OBJECT *f= gl_find_next(q,NULL);
    OBJECT *j;

	static FINDLIST *r=NULL;
    r=gl_find_objects(FL_NEW,FT_CLASS,SAME,"generator_controller",FT_END);
    OBJECT *ff= gl_find_next(r,NULL);
    OBJECT *jj;

	 for(j=f;j!=NULL;j=j->next)
   	 {


			 if(gl_object_isa(j,"windturb_dg")||gl_object_isa(j,"diesel_dg"))
			 {
				{
						double *price=gl_get_double_by_name(j,"price");
						int16 *give_only_reactive=gl_get_int16_by_name(j,"give_only_reactive");

						if(*price> current_frame.clearing_price)
						{
							if(*give_only_reactive==0)
							{
								for(jj=ff;jj!=NULL;jj=jj->next)
								{
								   if(gl_object_isa(jj,"generator_controller"))
								   {
									   if(!strcmp(jj->parent->name,j->name))
									   {
										  printf("close generator %s\n",j->name);
										  gl_set_value_by_name(jj,"generator_state","0");
			 ...
			 else if(gl_object_isa(j,"diesel_dg"))
			 {
				{
						double *price=gl_get_double_by_name(j,"price");
						int16 *give_only_reactive=gl_get_int16_by_name(j,"give_only_reactive");
...
if(cleared_frame.seller_total_quantity < cleared_frame.buyer_total_quantity)
{
    ...
    static FINDLIST *rr=NULL;
    rr=gl_find_objects(FL_NEW,FT_CLASS,SAME,"generator_controller",FT_END);
    OBJECT *fff= gl_find_next(rr,NULL);
    OBJECT *jjj;

					for(jjj=fff;jjj!=NULL;jjj=jjj->next)
					{
						   if(gl_object_isa(jjj,"generator_controller"))
						   {
							      printf("open generator %s",jjj->name);
								  gl_set_value_by_name(jjj,"generator_state","1");
...
*/
/////////////////////STORE THE MORE ENERGY IN BATTERIES ////////////////////////////////
if(cleared_frame.seller_total_quantity > cleared_frame.clearing_quantity)
{

  static FINDLIST *p=NULL;
  p=gl_find_objects(FL_NEW,FT_CLASS,SAME,"triplex_meter",FT_END);
  OBJECT *firstp= gl_find_next(p,NULL);
  OBJECT *l;

  static FINDLIST *pp=NULL;
  pp=gl_find_objects(FL_NEW,FT_CLASS,SAME,"transformer",FT_END);
  OBJECT *firstpp= gl_find_next(pp,NULL);
  OBJECT *ll;

  double *active_of_each_central_meter;
  double total_active_of_devices=0;

  for(l=firstp;l!=NULL;l=l->next)
  {
   		if(gl_object_isa(l,"triplex_meter"))
   		{

   			 int16 *central=gl_get_int16_by_name(l,"central");

   				 if((*central==1))
				 {
					 active_of_each_central_meter= gl_get_double_by_name(l,"measured_real_power");  //se Watt

								for(ll=firstpp;ll!=NULL;ll=ll->next)
								{
										if(gl_object_isa(ll,"transformer"))
										{

											 char buffer[1024];
											 char buffer2[1024];
											 gl_get_value_by_name(ll,"to",buffer,sizeof(buffer)); //exeit to onoma tou triplex_meter

											 if(!strcmp(buffer,l->name))
											 {

												 gl_get_value_by_name(ll,"from",buffer2,sizeof(buffer2)); //to buffer2 exei to onoma tou meter

												 static FINDLIST *ppp=NULL;
												 ppp=gl_find_objects(FL_NEW,FT_CLASS,SAME,"battery",FT_END);
												 OBJECT *firstppp= gl_find_next(ppp,NULL);
												 OBJECT *lll;

												 for(lll=firstppp;lll!=NULL;lll=lll->next)
												 {

													if(gl_object_isa(lll,"battery"))
													{
														if(!strcmp(lll->parent->name,buffer2))
														{
														//printf("active: %s %f\n",buffer2,fabs(*active_of_each_central_meter));
															//if(*active_of_each_central_meter < 0)
															//{

															//active_of_each_central_meter= gl_get_double_by_name(l,"measured_real_power");  //se Watt
															//printf("it's negaticve %f \n",*active_of_each_central_meter);
																char buf[2045];
																sprintf(buf,"%f",-(*active_of_each_central_meter));
																printf("active: %f\n",*active_of_each_central_meter);
																gl_set_value_by_name(lll,"scheduled_power",buf);
															//}
														}
													}
												 }
	...
	total_active_of_devices=total_active_of_devices + *(active_of_each_central_meter);
