			...
			if (gen_mode == STANDALONE)
			{
				...
				for (index=0; index<bid_curve_values[latency_write_section].number_bid_curve_sections; index++)
				{
					...
					if((gl_object_isa(parent2,"diesel_dg","generators")))
					{
						...
						//printf("bid_diesel_dg:%f ",(*bid_power_active)/1000);
					}
					else if(gl_object_isa(parent2,"windturb_dg","generators"))
					{
						...
						bid_power_reactive=gl_get_double_by_name(parent2,"TotalReacPow");
						bid_price=gl_get_double_by_name(parent2,"price");
						//printf("bid_windturb_dg:%f ",*bid_power_active);
					...
					if (gen_state != GEN_OFF)	//Generator is not off, so must be starting/started
					{
						...
						else if(gl_object_isa(parent2,"battery","generators"))
						{
							...
							//printf("test: %f  %f\n",*bid_power_active,(*E_max*20)/100);
							if((*bid_power_active)<(*E_max*20)/100)
							{
								//printf("buyer");
								bidID_val=auction_object->submit(obj,-(*bid_power_active)/1000,60,-1,BS_ON);
							}
							else
							{
								//printf("seller\n");
							    bidID_val=auction_object->submit(obj,(*bid_power_active)/1000,50,-1,BS_ON);
							}
						}

						else
						{
							//bid_curve_values[latency_write_section].Curve_Info[index].price=50;
						    ///printf("firsst_bid_diesel_dg:%f ",((*bid_power_reactive)/1000));
						    //if(bid_from_now==1){	
								double contr_price=*bid_price;
								bidID_val=auction_object->submit(obj,((*bid_power_active)/1000),contr_price,-1,BS_ON);
								//bidID_val2=auction_reac_object->submit(obj,((*bid_power_reactive)/1000),50,-1,BS_ON);
							///}
						}					
					}
					...
					{	
						...
						if(gl_object_isa(parent2,"solar","generators"))
						{
							...
							//bidID_val=auction_object->submit(obj,(((*complex_power).Im())/1000),bid_curve_values[latency_write_section].Curve_Info[index].price,-1,BS_OFF);														
						}
						else if(gl_object_isa(parent2,"battery","generators"))
						{							
							bidID_val=auction_object->submit(obj,((*bid_power_active)/1000),bid_curve_values[latency_write_section].Curve_Info[index].price,-1,BS_OFF);						
						}
						else 
						{
							//bid_curve_values[latency_write_section].Curve_Info[index].price=50;
							///printf("second_bid_diesel_dg:%f ",((*bid_power_reactive)/1000));
							//printf("GEN_OFF_ %s %f %f\n",OBJECTHDR(this)->name,temp_power_value,bid_curve_values[latency_write_section].Curve_Info[index].price);
							///if(bid_from_now==1)
							//{
							...
							//bidID_val2=auction_reac_object->submit(obj,((*bid_power_reactive)/1000),50,-1,BS_OFF);						
						}					
					}
					//Store the bidID		
				}
			...
			else	//Must be building mode
			{
				...
				else	//Only 1
				{
					...
					if((gl_object_isa(parent2,"diesel_dg","generators")))
					{
						...
						bid_power_reactive=gl_get_double_by_name(parent2,"TotalReacPow");
						bid_price=gl_get_double_by_name(parent2,"price");
						//printf("bid_diesel_dg:%f ",(*bid_power_reactive)/1000);
					}
					else if(gl_object_isa(parent2,"windturb_dg","generators"))
					{
						bid_power_active=gl_get_double_by_name(parent2,"TotalRealPow");
					...
					else if(gl_object_isa(parent2,"solar","generators"))
					{
						...
						complex_power=gl_get_complex_by_name(parent2,"VA_Out");
						bid_price=gl_get_double_by_name(parent2,"price");
						//printf("bid_solar:%f ",(*complex_power)/1000);
					...
					if (gen_state == GEN_OFF)
					{
						...
						if(gl_object_isa(parent2,"solar","generators"))
						{
							double contr_price=*bid_price;
							bidID_val=auction_object->submit(obj,(-(*complex_power).Re())/1000,contr_price,-1,BS_ON);
							//bidID_val2=auction_reac_object->submit(obj,(-(*complex_power).Im())/1000,bid_curve_values[latency_write_section].Curve_Info[0].price,-1,BS_ON);
												
						}
						else if(gl_object_isa(parent2,"battery","generators"))
						{
							
							bidID_val=auction_object->submit(obj,-(*bid_power_active)/1000,bid_curve_values[latency_write_section].Curve_Info[0].price,-1,BS_ON);
						
						}
						else
						{
							//printf("third_bid_diesel_dg:%f ");
							//	printf("gen_off %s %f %f\n",OBJECTHDR(this)->name,temp_power_value,bid_curve_values[latency_write_section].Curve_Info[0].price);
							//if(bid_from_now==1){
								
								double contr_price=*bid_price;
								bidID_val=auction_object->submit(obj,-(*bid_power_active)/1000,contr_price,-1,BS_ON);
							//}
							//bidID_val2=auction_reac_object->submit(obj,-(*bid_power_reactive)/1000,bid_curve_values[latency_write_section].Curve_Info[0].price,-1,BS_ON);
					...
					{	//Must be on or starting, so building is "off"
						...
						else if(gl_object_isa(parent2,"battery","generators"))
						{
							...
							bidID_val=auction_object->submit(obj,-(*bid_power_active)/1000,bid_curve_values[latency_write_section].Curve_Info[0].price,-1,BS_OFF);
						
						}
						else
						{
							//	printf("forth_bid_diesel_dg:%f ");
							//if(bid_from_now==1)
							//{
							...
							//}
						//bidID_val2=auction_reac_object->submit(obj,-(*bid_power_reactive)/1000,bid_curve_values[latency_write_section].Curve_Info[0].price,-1,BS_OFF);
